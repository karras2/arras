...
// Define player keys
var keys = [
  "TOKEN_kjsfhgwoehHUIGUFOIvdgkhUSGFjcn_TOKEN"
];
  ...
  buildList(range) {
    ...
    let out = entities.map(e => {
      ...
      if (e.health.amount > 0) {
        if (!e.invuln) {
          if (e.master.master.team !== this.body.master.master.team && e.alpha > 0.5) {
      ...
      const broadcast = (() => {
        ...
        const getminimap = (() => {
              ...
              (() => {
                ...
                return () => {
                  ...
                  let clean = publicmap.update(minimap.map(function(entry) {
                    return [entry[1], entry[2], (entry[4] === 'miniboss' || entry[4] === 'tank') ? entry[3] : 17];
                  }));
          ...
          return () => {
            // Update the minimap
            entities.forEach((my) => {
              if (my.settings.drawShape && ran.dice(my.stealth * c.STEALTH) && (my.type === "tank" || my.type === "miniboss" || my.type === "wall")) {
                let i = minimap.findIndex((entry) => {
...
var maintainloop = (() => {
    ...
    return census => {
      if (timer > 100) {
        ...
        switch (ran.chooseChance(20, 15, 10, 10, 1, 1, 1)) {
          case 0:
            choice = [[Class.elite_destroyer, Class.elite_gunner, Class.elite_sprayer, Class.elite_battleship], 1, 'a', 'nest'];
            break;
          case 1:
            choice = [[Class.palisade, Class.skimboss, Class.summoner, Class.nestKeeper], 1, 'castle', 'norm'];
            sockets.broadcast('A strange trembling...');
            break;
          case 2:
            choice = [[Class.elite_destroyer, Class.elite_gunner, Class.elite_sprayer, Class.elite_battleship], 2, 'a', 'nest'];
            break;
          case 3:
            choice = [[Class.palisade, Class.skimboss, Class.summoner, Class.nestKeeper], 2, 'castle', 'norm'];
            sockets.broadcast('A strange trembling...');
            break;
          case 4:
  ...
  let spawnCrasher = census => {
    if (ran.chance(1 - 0.5 * census.crasher / (room.maxFood * 200) / room.nestFoodAmount)) {
      let spot,
          i = 30;
      do {
  ...
  let makenpcs = (() => {
    ...
    return () => {
      ...
      if (bots.length < c.BOTS && Math.random() > 0.8) {
        ...
        o.define(Class.bot);
        o.define(Math.random() > 0.75 ? Class.betaTanks : Class.basic);
        ...
        if (c.MODE === "tdm") {
          ...
          TEAMS = TEAMS.sort(function(a, b) { return a[1] - b[1] });
          o.team = TEAMS[0][0];
